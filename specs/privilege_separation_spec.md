...
2.  **Lifecycle Transitions**:
    ...
    *   **Stop**: The parent sends a termination signal (`SIGTERM` or `SIGKILL`) to the worker's PID, reaps it via the orphan reaper loop to prevent zombie processes, and closes the associated IPC and socket descriptors.
    *   **Reconfigure / Restart** (e.g., during a LAN/WAN subnet shift): The parent kills the active worker process, tears down the old interface parameters, opens new socket descriptors matching the updated configuration, and spawns a fresh worker instance.
3.  **Process Supervision**: The parent process tracks active child worker PIDs. If a child process terminates unexpectedly (e.g., due to a panic, segmentation fault, or out-of-memory kill), the parent's reaper loop traps the `SIGCHLD` signal, cleans up associated states (such as routes or IP leases), and executes a restart strategy (e.g., spawning a fresh instance with exponential back-off).
